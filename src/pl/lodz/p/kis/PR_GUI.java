...
public class PR_GUI extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        selbox_nfeat.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "1", "2", "3" }));
...
//    	List<Matrix> splitResult= splitObjects();
//    	Matrix classifiedObjects = splitResult.get(0);
...
//    	System.out.println(classifiedObjects.getRowDimension());
    ...
    */
    ...
    private void selectFeatures(int[] flags, int d) {
    	...
    	averagesA = extractAveragesForOneClass(computeAveragesForAllFeatures(), 0);
    	averagesB = extractAveragesForOneClass(computeAveragesForAllFeatures(), 1);
    ...
    /**
     * Metoda liczy srednia wartosc danej cechy ze wszystkich obiektow.
     * Zwraca wektor zawierajacy srednie wartosci cech dla dwoch klas. 
     */
    ...
    /**
     * Metoda zwraca macierz zawierajaca w usrednione wartosci wszystkich cech dla wszystkich klas.
     * Wymiar macierzy iloss_cech x ilosc_klas
     */
    ...
    /**
     * Metoda zwraca macierz zawierajaca powielone usrednione wartosci wszystkich cech dla jednej klasy.
     * Macierz jest wykorzystywana do ułatwienia obliczeń w rozrzucie.
     */
    ...
    private Matrix computeClassSpread(double[][] features, double[] avgVector) {
    	...
    	Matrix averagesMatrix = this.cloneAverages(avgVector, features[0].length);
     ...
     */
    private double[] extractAveragesForOneClass(double[][] averages, int classIndex) {
     ...
     */
    private double[][] extractFeaturesMatrixForOneClass(double[][] features, int classIndex) {
    	double[][] result = new double[FeatureCount][];
     ...
     */
    private double computeFisherFactor(double[] mA, double[] mB, double[][] a, double[][] b) {
    ...
    private void computeSfs(int numberOfFeatures) {
    	...
    	for (int i = 1; i <= numberOfFeatures; i++) {
        	...
        	for (int[] indices : generatedIndices) {
    			...
    			currentFisherFactor = computeFisherFactor(selectedAveragesA, selectedAveragesB, selectedFeaturesA, selectedFeaturesB);
		...
		System.out.println("[SFS - Stop] Execution time: " + (methodStopTime - methodStartTime) + " miliseconds \n");
    }
    
    
    
    /**
     * Liczy współczynnik fishera klasyczną metodą. 
     * @param numberOfFeatures ilość wybranych cech.
     */
    private void computeFisherLD(int numberOfFeatures) {
		...
		System.out.println(maxFisherFactor);
		double methodStopTime = System.currentTimeMillis();
		System.out.println("[Classic - Stop ] Execution time: " + (methodStopTime - methodStartTime) + " miliseconds \n");
...
//        double[][] C = new double[M.length][M.length];
    ...
    /**
     * Tworzymy wektor reprezenetujacy jeden obiekt.
     * W macierzach informacje sa trzymane w postaci CECHY (wiersze) x OBIETY (kolumny), 
     * a do dalszych obliczeń przydadzą się informacje o samych obiektach, 
     * więc do tematu trzeba podejść od dupy strony.
     * 
     * @param index - indeks z macierzy
     * @param objectsMatrix - macierz z danymi
     */
    protected double[] extractObject(double[][] objectsMatrix, int index) {
    	double[] result = new double[FeatureCount];
    	for (int i = 0; i < FeatureCount; i++) {
    ...
    private abstract class Classifier {
    	...
    	protected List<FileObject> allObjects = new ArrayList<>();
    	protected List<FileObject> classifiedObjects =  new ArrayList<>();
    	protected List<FileObject> unclassifiedObjects =  new ArrayList<>();
  	
    	//posłuży później do porównania ile obiektów dobrze zostało sklasyfikowanych
    	protected List<FileObject> unclassifiedObjectsCopy =  new ArrayList<>(); 
		...
		private void generateTrainingSets() {
			...
			double numberOfClassifedObjects = numberOfAllObjects * Integer.valueOf(tf_TrainSetSize.getText()) / 100.0;
			int ratio = (int) (numberOfClassifedObjects / (numberOfAllObjects - numberOfClassifedObjects)) + 1;

			for (int i = 0; i < ClassLabels.length; i++) {
				if (ClassLabels[i] == 0)
					allObjects.add(new FileObject(extractObject(featuresA, i),
							FileObject.CLASS_A, i));
				else
			...
			try {
				for (int i = 0; i < numberOfAllObjects; i++) {
					if (i % ratio == 0) {
						FileObject o = (FileObject) allObjects.get(i).clone();
						o.setClassType(FileObject.UNCLASSIFIED);
						unclassifiedObjects.add(o);
			...
			double methodStopTime = System.currentTimeMillis();
	    	System.out.println("[generateTrainingSets - stop] Execution Time: " + (methodStopTime - methodStartTime) + " miliseconds");
		}
		...
		protected double computeLength(FileObject classified, FileObject unclassified) {
			double vectorLength = 0.0;
			int featureIndex = 0;
			for (int i = 0; i < selectedFeaturesIndices.length; i++) {
				featureIndex = selectedFeaturesIndices[i];
				double difference = classified.getFeatures()[featureIndex] - unclassified.getFeatures()[featureIndex];
    ...
    private class NNClassifier extends Classifier {
    	...
    	public void execute() {
    		...
    		int currentIndex = 0;
       		...
       		for (FileObject unclassified : unclassifiedObjects) {
    			for (FileObject classified : classifiedObjects) {
    				if (currentIndex++ == 0) {
    					minLength = computeLength(classified, unclassified);
    					unclassified.setClassType(classified.getClassType());
    				}
    				else {
    					currentLength = computeLength(classified, unclassified);
       		...
       		for (int i = 0; i < unclassifiedObjects.size(); i++) {
       			if (unclassifiedObjects.get(i).compareTo(unclassifiedObjectsCopy.get(i)) == 0)
       				properlyClassified++;
       		}
       		
       		System.out.println("[NNClassifier] Properly classified objects: " + (properlyClassified / Double.valueOf(unclassifiedObjects.size())) * 100 + "%");
    	}
    }
    
    private class KNNClassifier extends Classifier {
    	Map<FileObject, Double> selectedDistances = new HashMap<>();
		...
		public void execute() {
    		double currentLength = 0.0;
       		...
       		for (FileObject unclassified : unclassifiedObjects) {
    			for (FileObject classified : classifiedObjects) {
    				currentLength = computeLength(classified, unclassified);
    				selectedDistances.put(classified, currentLength);
    			}
    			fitTheClass(unclassified);

    		}
       		
       		// Sprawdzamy ile udało się dobrze przyporządkować
       		for (int i = 0; i < unclassifiedObjects.size(); i++) {
       			if (unclassifiedObjects.get(i).compareTo(unclassifiedObjectsCopy.get(i)) == 0)
       				properlyClassified++;
       		}
       		
       		System.out.println("[kNNClassifier] Properly classified objects: " + (properlyClassified / Double.valueOf(unclassifiedObjects.size())) * 100 + "%");
    	}
		
		private void fitTheClass(FileObject o) {
			Object[] keys = new Object[userFeatures];
			ValueComparator comparator = new ValueComparator(selectedDistances);
			TreeMap<FileObject, Double> sortedMap = new TreeMap<FileObject, Double>(comparator);
			int classACount = 0;
			int classBCount = 0;
			
			sortedMap.putAll(selectedDistances);
			keys = sortedMap.keySet().toArray();
			
			for (int i = 0; i < userFeatures; i++) {
				FileObject key = (FileObject) keys[i];
				if (key.getClassType() == FileObject.CLASS_A)
					classACount++;
				else
					classBCount++;
			}
			...
			if (classACount > classBCount)
				o.setClassType(FileObject.CLASS_A);
			else
				o.setClassType(FileObject.CLASS_B);
			
		}
		
		// Komparator do sortowania mapy
		private class ValueComparator implements Comparator<FileObject> {
			Map<FileObject, Double> base;
		  
			public ValueComparator(Map<FileObject, Double> base) {
		        this.base = base;
		    }
